...
export type DirectoryPickerEnv = Readonly<
  Partial<Record<'SSH_CONNECTION' | 'SSH_TTY' | 'DISPLAY' | 'WAYLAND_DISPLAY' | 'DSH_DIRECTORY_PICKER' | 'DSH_HEADLESS', string>>
>
...
export interface DirectoryPickerHostFacts {
  ...
  /** Environment sample; SSH marks remote operator, DISPLAY/WAYLAND Linux display, DSH_* explicit overrides. */
...
/**
 * Resolve which backend serves this boot. Explicit environment overrides
 * (`DSH_DIRECTORY_PICKER=browse` or `DSH_HEADLESS=true`) force `browse`.
 * Otherwise, `native` requires every signal that the operator can see the host
 * display and the native backend can serve it: a loopback-only bind (an
 ...
 * server), and a servable display session — assumed on darwin/win32, requiring
...
export function resolveDirectoryPickerBackend(facts: DirectoryPickerHostFacts): DirectoryPickerBackendKind {
  if (facts.env.DSH_DIRECTORY_PICKER === 'browse' || facts.env.DSH_HEADLESS === 'true') return 'browse'
